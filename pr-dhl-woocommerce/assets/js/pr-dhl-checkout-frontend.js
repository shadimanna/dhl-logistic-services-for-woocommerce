jQuery(function ($) {
  ...
  $doc.on('updated_checkout', function () {
    ...
    if (pr_dhl_checkout_frontend.cod_enabled) {
      jQuery('.woocommerce').
        on('change', '.payment_methods .input-radio', function () {
          $body.trigger('update_checkout')
        })
    }
...
function gm_authFailure () {
  ...
  jQuery('.woocommerce-checkout #dhl_parcel_finder_form #dhl_google_map').
    before(pr_dhl_checkout_frontend.no_api_key)
}
...
jQuery(document).ready(function ($) {
  var wc_checkout_dhl_parcelfinder = {
    ...
    init: function () {
      ...
      $(document.body).
        on('click', '#dhl_parcel_finder', this.init_form)

      $(document.body).
        on('click', '.parcelshop-select-btn', this.selectedShop)

      $(document.body).
        on('change', '#shipping_dhl_address_type', this.address_type)

      $(document.body).
        on('change', '#shipping_dhl_drop_off', this.selectedDropOff)
    ...
    address_type: function () {
      var address_type = $('.woocommerce-checkout #shipping_dhl_address_type').
        val()
      ...
      if (address_type == 'dhl_packstation') {
        ...
        if (!$('.woocommerce-checkout #shipping_dhl_postnum_field label span').length ||
            (
              !$('.woocommerce-checkout #shipping_dhl_postnum_field label span').
                hasClass('required')
            )) {
          $('.woocommerce-checkout #shipping_dhl_postnum_field label').
            append(' <span class="required">*</span>')
        }
      } else if (address_type == 'dhl_branch') {
        $('.woocommerce-checkout #shipping_dhl_postnum_field').show()
        // remove "required" span tag
        $('.woocommerce-checkout #shipping_dhl_postnum_field label .required').
          remove()
      } else {
    ...
    init_form: function () {
      ...
      if ($('.woocommerce-checkout #billing_country').length) {
        gmap_country = $('.woocommerce-checkout #billing_country').
          val()
      }
      ...
      if ($('.woocommerce-checkout #billing_postcode').length) {
        gmap_postcode = $('.woocommerce-checkout #billing_postcode').
          val()
      }
      ...
      if ($('.woocommerce-checkout #billing_address_1').length) {
        gmap_address_1 = $('.woocommerce-checkout #billing_address_1').
          val()
      }
      var gmap_address_2 = ''
      if ($('.woocommerce-checkout #billing_address_2').length) {
        gmap_address_2 = $('.woocommerce-checkout #billing_address_2').
          val()
      }
    ...
    submit: function () {
      ...
      $('#dhl_parcel_finder_form #checkout_dhl_parcel_finder .woocommerce-error').
        remove()
      ...
      if (!pf_post_code) {
        $('#dhl_parcel_finder_form #checkout_dhl_parcel_finder').
          append('<div class="woocommerce-error">' + pr_dhl_checkout_frontend.post_code_error + '</div>')
        return false
      }
      ...
      var data = {
        ...
        packstation_filter: $('#dhl_packstation_filter').
          is(':checked'),
        branch_filter: $('#dhl_branch_filter').is(':checked'),
        security: $form.find('input[name="dhl_parcelfinder_nonce"]').
          val(),
      }
        ...
        success: function (parcelShopsJSON) {
          ...
          if (parcelShopsJSON) {
            ...
            if (parcelShopsRes.error) {
              $('#dhl_parcel_finder_form #checkout_dhl_parcel_finder').
                append('<div class="woocommerce-error">' + parcelShopsRes.error + '</div>')
            } else {
    ...
    populateDropdown: function () {
      ...
      var data = {
        ...
        packstation_filter: $('#dhl_packstation_filter').
          is(':checked'),
        branch_filter: $('#dhl_branch_filter').is(':checked'),
        security: $('form#checkout_dhl_parcel_finder').
          find('input[name="dhl_parcelfinder_nonce"]').
          val(),
      }
        ...
        if (value.location.ids[0].locationId == parcelShopId) {
          ...
          switch (value.location.type) {
            case 'locker':
              ...
              $('.woocommerce-checkout #shipping_dhl_address_type').
                val('dhl_packstation').
                trigger('change')
              break
            case 'servicepoint':
    ...
    populateOsmMap: function () {
      ...
      wc_checkout_dhl_parcelfinder.osmMap = L.map('dhl_google_map').
        setView(uluru, 13)
